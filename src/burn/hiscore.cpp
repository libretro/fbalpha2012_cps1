...
void HiscoreInit(void)
{
   ...
   snprintf(szDatFilename, sizeof(szDatFilename),
         "%s%cfbalpha2012%chiscore.dat",
...
void HiscoreReset()
{
	...
	for (UINT32 i = 0; i < nHiscoreNumRanges; i++)
   {
      ...
      if (HiscoreMemRange[i].Loaded)
      {
         ...
         if (HiscoreMemRange[i].NumBytes > 1)
            cpu_write_byte(HiscoreMemRange[i].Address + HiscoreMemRange[i].NumBytes - 1, (UINT8)~HiscoreMemRange[i].EndValue);
         cpu_close();
      }
   }
}
