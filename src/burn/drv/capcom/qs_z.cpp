...
static INT32 QsndZBankMap(void)
{
   UINT32 nOff;
   UINT8* Bank;
   nOff = nQsndZBank << 14;
...
INT32 QsndZInit(void)
{
	...
	if (Cps1Qs)
   {
		...
		ZetMapArea(0x0000, 0x7FFF, 2, CpsZRom, CpsZRom - (nCpsZRomLen / 2));
      /* If it tries to fetch this area */
	}
   else
   {
	...
	if (Cps1Qs)
   {
		ZetMapArea(0xD000, 0xEFFF, 2, CpsZRom, CpsZRom - (nCpsZRomLen / 2));
      /* If it tries to fetch this area */
	}
   else
