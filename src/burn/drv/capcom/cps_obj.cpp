...
INT32 CpsObjInit(void)
{
   INT32 i;
	...
	nFrameCount = 2;			/* CPS2 sprites lagged by 1 frame 
                              and double buffered
...
INT32 DaimakaibObjGet(void)
{
   ...
   /* writes a blank sprite, followed by end 
    * of sprite list marker, start at 0x10 to ignore these */
...
INT32 FcrashObjDraw(INT32 nLevelFrom,INT32 nLevelTo)
{
	...
	for (i=0; i<pof->nCount; i++,ps+=nPsAdd)
   {
      ...
      INT32 n = BURN_ENDIAN_SWAP_INT16(ps[0]);
      INT32 a = BURN_ENDIAN_SWAP_INT16(ps[1]);
      INT32 x = BURN_ENDIAN_SWAP_INT16(ps[2]);
      INT32 y = BURN_ENDIAN_SWAP_INT16(ps[3]);
